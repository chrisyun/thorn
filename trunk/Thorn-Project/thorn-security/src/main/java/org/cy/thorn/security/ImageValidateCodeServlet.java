...
public class ImageValidateCodeServlet extends HttpServlet {
	...
	private static Logger log = Logger.getLogger(ImageValidateCodeServlet.class);
	/**
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		// 定义图片的宽度和高度
		int width = SecurityConstant.AUTHCODE_IMAGE_WIDTH;
		int height = SecurityConstant.AUTHCODE_IMAGE_HEIGHT;
		// 创建一个图像对象
		...
		for (int i = 0; i < SecurityConstant.AUTHCODE_NUM_LENGTH; i++) {
			String rand = RandomUtil.randomString(1);
		...
		// 将生成的随机字符串写入Session
		request.getSession(true).setAttribute(SecurityConstant.AUTHCODE_SESSION_ID,
