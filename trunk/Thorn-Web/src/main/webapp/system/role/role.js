...
Ext
		.onReady(function() {
			...
			var remarkRender = function(remark, metadata, record, rowIndex,
					colIndex) {
			...
			var recordArray = [
					...
					getRecord("是否禁用", "isDisabled", "string", 50, true,
							yesOrNoRender),
					getRecord("描述", "roleDesc", "string", 200, false,
							remarkRender),
					getRecord("授权", "authColumn", "string", 50, false,
							authRender),
					getRecord("角色成员", "roleUserColumn", "string", 70, false,
							userRender) ];
			role_grid = new GridUtil(rolePageUrl, recordArray, pageSize);

			var grid_Bar = getCommonBar(saveHandler, modifyHandler,
					deleteHandler);
			...
			/** ****************role Grid panel end************ */
			var grid = role_grid.getGridPanel();
			var store = role_grid.getStore();

			/** ****************org window start************ */
			var role_form = new FormPanel( {
				id : "roleForm",
				collapsible : false,
				labelWidth : 80,
				border : false
			...
			role_form
					.addComp(getTextArea("roleDesc", "描述", 180, 60), 1.0, true);
			role_form.addComp(getComboBox("isDisabled", "是否禁用", 180, yesOrNo,
					false), 1.0, false);
			...
			function saveOrModify() {
				...
				var ajaxClass = new AjaxUtil(roleSaveOrModifyUrl);
						...
						.confirm(
								...
								function(btn) {
									if (btn == "yes") {
										...
										var ajaxClass = new AjaxUtil(
												roleDeleteUrl);
										ajaxClass.request(params, true, null,
												function(obj) {
